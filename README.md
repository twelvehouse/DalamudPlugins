# Dalamud Plugin Repository

Custom Dalamud plugin repository by [twelvehouse](https://github.com/twelvehouse).

## Setup

Add the following URL to **Dalamud → Settings → Experimental → Custom Plugin Repositories**:

```
https://raw.githubusercontent.com/twelvehouse/DalamudPlugins/main/pluginmaster.json
```

## Plugins

| Name | Description | Version |
|------|-------------|---------|
| ChatAnywhere | Host your chat on a web server so you can talk from anywhere. | 0.1.6.0 |
| FootIK | Real-time foot IK for FFXIV characters. Keeps feet planted naturally on terrain (stairs, slopes, uneven ground) during normal gameplay without requiring GPose or Brio. | 1.0.0.0 |
